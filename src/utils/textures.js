/**
 * Placeholder texture factory functions.
 *
 * These generate fallback textures used when asset files are missing,
 * so the game is playable even before all art is loaded.
 * Extracted from BootScene as importable pure functions.
 */
...
export function createPlaceholderTextures(scene) {
  ...
  // 1×1 white pixel (useful for debug drawing)
  g.fillStyle(0xffffff, 1);
  g.fillRect(0, 0, 2, 2);
  g.generateTexture('pixel', 2, 2);
  g.clear();

  // Circle primitive for particle emitters
  g.fillStyle(0xffffff, 1);
  g.fillCircle(8, 8, 8);
  ...
  // Player placeholder (only if real sprite not loaded)
  ...
  // Keg placeholder (only if real sprite not loaded)
